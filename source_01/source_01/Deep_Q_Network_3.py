...
writer = SummaryWriter(log_dir='graph/Deep_Q_Network_3')
...
optimizer = optim.Adam(filter(lambda p: p.requires_grad, dqn.parameters()), lr=learning_rate)
criterion = nn.MSELoss()

# 마지막 layer의 weight를 distance로 초기화
with torch.no_grad():
    q_weights = torch.tensor(distances, dtype=torch.float32).to(device)
    dqn.fc3.weight = nn.Parameter(q_weights)
    dqn.fc3.bias = nn.Parameter(torch.zeros(action_dim).to(device))
    dqn.fc3.weight.requires_grad = False
    dqn.fc3.bias.requires_grad = False
...
for episode in tqdm(range(num_episodes)):
    ...
    print(f"\nLoss: {loss:.6f}, Rewards: {episode_reward:.2f}")
