...
STEP_SIZE = 100                     # 학습률 감소 주기 (에포크 수)
LR_DC_RATE = 0.1 ** (1 / 100)       # learning rate 감소 비율
LOG_DIR = 'graph/DNN_Q_Learning_2'
...
# loss 계산을 위한 가중치 스케줄링
def get_alpha_beta(epoch, total_epochs, alpha_start, alpha_end, beta_start, beta_end):
    alpha = alpha_start - (alpha_start - alpha_end) * (epoch / total_epochs)
...
scheduler = optim.lr_scheduler.ExponentialLR(optimizer, gamma=LR_DC_RATE)
...
for epoch in tqdm(range(EPOCHES)):
    ...
    loss_mse = nn.functional.mse_loss(output_q_table, target_q_table)
    path_cost_normalized = path_cost * PATH_COST_SCALING_FACTOR
    alpha, beta = get_alpha_beta(epoch, EPOCHES, alpha_start, alpha_end, beta_start, beta_end)
    loss = alpha * loss_mse + beta * (path_cost_normalized)
    ...
    # 예측 cost가 이전까지 구한 best_cost보다 작아지면 best_q_table 교체
    if path_cost < best_cost:
        best_cost = path_cost
        best_path = optimal_path

print(f'\nBest Path Distance: {best_cost:.4f}')
print('\nBest Path After Training:')
print_partial_path(best_path)




''' solution 저장 시 주석 해제, 현재까지 "best : 25.0250"이 저장되어 있음 '''
# # solution path 저장
# save_path_to_csv(best_path)
